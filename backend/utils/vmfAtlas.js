...
function getVMFBoundsFromText(vmfContent) {
    ...
    if (!isFinite(minX)) {
        return { minX: 0, maxX: 128, minY: 0, maxY: 128, minZ: 0, maxZ: 128, width: 128, height: 128, depth: 128 }
    }
...
function offsetVMFText(vmfContent, offsetX, offsetY, offsetZ) {
    ...
    result = result.replace(/"plane"\s+"\(([^)]+)\)\s+\(([^)]+)\)\s+\(([^)]+)\)"/g, (match, p1, p2, p3) => {
...
async function mergeVMFsIntoGrid(vmfFiles, outputPath, options = {}) {
    ...
    for (const file of vmfFiles) {
        ...
        if (content.includes('NaN')) {
            console.error(`  ❌ VMF contains NaN values: ${file.name} (skipping)`)
            continue
        }
    ...
    return new Promise((resolve, reject) => {
        const pythonProcess = spawn('python', [pythonScript, '--json', configPath], {
        ...
        pythonProcess.on('close', (code) => {
            ...
            if (code !== 0) {
                ...
                return reject(new Error(`VMF merge failed: ${stderr || 'Unknown error'}`))
            }
            ...
            try {
                ...
                console.log(`   Grid: ${layoutData.cols}×${layoutData.rows}, Cell size: ${Math.round(layoutData.cellSize)} (square)`)
                ...
                console.error(`  ❌ Failed to parse merge result:`, parseError.message)
                reject(new Error(`Failed to parse merge result: ${parseError.message}`))
            }
        })
...
async function splitOBJByGrid(objPath, gridLayout, outputDir, cellSize = 256) {
    console.log(`✂️  Splitting combined OBJ into ${gridLayout.length} individual models...`)
    ...
    console.log(`  📊 Parsed OBJ: ${vertices.length} vertices, ${faces.length} faces, ${materials.length} materials`)
    ...
    for (const face of faces) {
        ...
        for (let i = 0; i < verts.length; i++) {
            for (let j = i + 1; j < verts.length; j++) {
                if (verts[i] >= 0 && verts[i] < vertices.length && 
                    verts[j] >= 0 && verts[j] < vertices.length) {
    ...
    console.log(`  🗳️  Assigning mesh components to grid cells by majority vote...`)
    ...
    for (let cIdx = 0; cIdx < clusters.length; cIdx++) {
        ...
        for (const vIdx of cluster) {
            ...
            for (let cellIdx = 0; cellIdx < gridLayout.length; cellIdx++) {
                ...
                if (sourceX >= b.minX && sourceX < b.maxX &&
                    sourceY >= b.minY && sourceY < b.maxY) {
                    votes[cellIdx]++
                    break
                }
            }
        ...
        for (let i = 0; i < votes.length; i++) {
            if (votes[i] > max) { max = votes[i]; win = i }
        }
    ...
    for (let cIdx = 0; cIdx < clusters.length; cIdx++) {
        ...
        for (let cellIdx = 0; cellIdx < gridLayout.length; cellIdx++) {
            ...
            if (d2 < bestDist) { bestDist = d2; best = cellIdx }
        }
    ...
    if (fragmentsReassigned) {
        console.log(`  🔄 Reassigned ${fragmentsReassigned} tiny mesh fragments to nearest cell centre.`)
    }
    ...
    for (let cellIdx = 0; cellIdx < gridLayout.length; cellIdx++) {
        ...
        console.log(`    Cell ${cellIdx} (${gridLayout[cellIdx].name}): ${count} vertices`)
    }
    ...
    for (const face of faces) {
        ...
        let majority = -1, max = 0
        for (const k in voteCount) {
            if (voteCount[k] > max) { max = voteCount[k]; majority = parseInt(k) }
        }
    ...
    for (let cellIdx = 0; cellIdx < gridLayout.length; cellIdx++) {
        ...
        for (const face of cellFaces) {
            ...
            const remapped = face.line.replace(/(\d+)(\/\d+)?(\/\d+)?/g, (match, vIdx, vt, vn) => {
                const oldIdx = parseInt(vIdx)
                const newVIdx = vertexMap.get(oldIdx)
                if (!newVIdx) {
                    console.warn(`    ⚠️  Vertex ${oldIdx} not in cluster map!`)
                    return match
                }
        ...
        if (mtlLib) {
            ...
            const combinedMtlPath = path.join(path.dirname(objPath), combinedMtlFile)
            
            if (fs.existsSync(combinedMtlPath)) {
                fs.copyFileSync(combinedMtlPath, splitMtlPath)
                console.log(`    ✅ Copied MTL: ${cell.name}.mtl`)
            } else {
                console.warn(`    ⚠️  Combined MTL not found: ${combinedMtlPath}`)
            }
        }
