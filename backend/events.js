...
function reg_events(mainWindow) {
    ...
    ipcMain.handle("save-staged-editoritems", async (event, { itemId, stagedEditorItems, hasObjFiles = false }) => {
        try {
            ...
            // Check if a custom model was added (starts with "bpee/") AND the model file actually exists AND OBJ files exist
            // Only mark as custom if OBJ files were found (meaning it was generated by BeePEE)
            const subType = stagedEditorItems?.Item?.Editor?.SubType
            const subTypes = Array.isArray(subType) ? subType : [subType]
            
            // Check all subtypes for custom models and verify files exist
            ...
            for (const st of subTypes) {
                ...
                if (typeof modelName === 'string' && modelName.startsWith('bpee/')) {
                    // Resolve the model path to check if file exists
                    // Model path is relative like "bpee/item/bpee_myitem_areng_a3f9.mdl"
                    // Need to check in resources/models/
                    const modelPath = path.join(item.packagePath, "resources", "models", modelName)
                    
                    if (fs.existsSync(modelPath) && hasObjFiles) {
                        // Model file exists AND OBJ files were found (meaning it was generated by BeePEE)
                        hasValidCustomModel = true
                        console.log(`✅ Found custom model file: ${modelPath} (OBJ files confirmed)`)
                        break // Found at least one valid model, that's enough
                    } else if (fs.existsSync(modelPath) && !hasObjFiles) {
                        // Model file exists but no OBJ files - don't mark as custom
            ...
            if (hasValidCustomModel) {
                // At least one custom model file exists and OBJ files were found, update metadata
    ...
    ipcMain.handle("copy-staged-model-files", async (event, { itemId }) => {
        try {
            ...
            // Check if OBJ files exist in staging directory (before cleanup)
            // This indicates the model was generated by BeePEE
